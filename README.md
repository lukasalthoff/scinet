...
SciNet organizes research work into its own hierarchy of domains, fields, and subfields, built independently rather than adopted from an existing classification. Every task is filed at the level it genuinely belongs to — universal tasks apply to research everywhere, domain and field tasks to a whole domain or field, and subfield tasks to one subfield — so a subfield inherits the broader tasks above it rather than repeating them.

Task statements are generated by large language models and validated against published papers, which are read in full and scored for whether the task was performed. The validation run that expanded the taxonomy in v1.3 kept a candidate task only where it appeared in at least 5% of a subfield's papers. `task_prevalence.csv` reports that observed share for every task on a larger pooled sample, so tasks carried over from earlier releases are included and some fall below 5%.

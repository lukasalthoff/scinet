...
| File | Description |
|------|-------------|
| `tasks.csv` | Every task in the hierarchy. Columns: `task`, `category`, `level`, `domain`, `field`, `subfield`. Level is one of "universal" (applies to all researchers), "domain" (e.g. Social Sciences), "field" (e.g. Economics), or "subfield" (e.g. Labor Economics). Higher-level columns are empty when a task belongs to a broader level. |
